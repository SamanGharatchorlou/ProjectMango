...
UISlider::UISlider(const StringMap32& attributes) : UIBox(attributes), xCursorOffset(0.0f), mSlider(nullptr), mBar(nullptr), mSelectable(true) { }
...
void UISlider::setComponents(Texture* container, UIButton* slider, UIBox* bar)
{
	...
	{
		sliderSize = mSlider->texture()->originalDimentions.realiseSize(mRect.Height() *1.35f);
	}
...
{
	position = Maths::clamp(position + xCursorOffset, mRect.LeftPoint(), mRect.RightPoint());
